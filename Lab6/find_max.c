...
int find_max_gpu(unsigned int *data, unsigned int length)
{
  ...
  printf("Each thread will output %d numbers per iteration for %d iterations for a total of a %d time total reduction\n", outputsPerThread, MAX_ITERATIONS, MAX_ITERATIONS*outputsPerThread);
  ...
  for(int iteration = 0; iteration < MAX_ITERATIONS; ++iteration) {
    ...
    for(int i = 0; i <= numberOfRuns; ++i) {
      ...
      ciErrNum = clEnqueueWriteBuffer(commandQueue, io_data, CL_TRUE, 0 , PART_SIZE*sizeof(unsigned int), data + numberOfRuns*PART_SIZE*sizeof(unsigned int) , 0, NULL, &eventWriteBuffer);
